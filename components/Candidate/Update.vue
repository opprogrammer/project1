			...
			<div
				...
				<div
					...
					<button
						...
						:data-modal-toggle="`updateRecord-${record._id}`">
						<svg
							aria-hidden="true"
							class="w-5 h-5"
							fill="currentColor"
					...
					<div class="grid gap-4 mb-8 sm:grid-cols-2">
						<div v-for="(header, i) in headers" :key="i">
							...
							<input
								...
								v-model="record[header.parent][header.field]"
								:type="header.type"
								:name="`add-${header.name}`"
								:id="`add-${header.name}`"
								class="bg-gray-50 border border-gray-300 text-gray-900 text-base rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
...
<script setup>
	...
	onBeforeMount(() => {
		...
		record.value._id = BSON.ObjectID(record.value._id);
		let tempDate = new Date(record.value.start_date);
		...
		record.value.start_date = [year, month, date].join("-");

		tempDate = new Date(record.value.end_date);
		...
		record.value.end_date = [year, month, date].join("-");
	});
	...
	const updateRecordHandler = () => {
		...
		record.value.full_name = `${record.value.first_name} ${record.value.last_name}`;
		record.value.start_date = new Date(record.value.start_date);
		record.value.end_date = new Date(record.value.end_date);

		console.log("test", record.value);
		emit("updateRecord", record.value);
	};
</script>
