<script setup>
	...
	const { TABLE_DATA, headers, groupMap, changeGroup, tableRowMap } =
		defineProps([
			"TABLE_DATA",
			"headers",
			"groupMap",
	...
	<div class="relative overflow-x-auto rounded-md">
		<table
			...
			<tbody v-if="!grouped.active" class="record-tbody">
				<tr class="text-base border-b cursor-pointer bg-gray-50 max-xl:text-sm"
					v-for="(data, index) in TABLE_DATA" :key="data._id">
					<CandidateTableRow
						:key="data.id"
						:data="data"
			...
			<tbody v-else>
				<template v-for="(records, group) in byGrouped" :key="group">
					...
					<draggable
						...
						itemKey="grouped"
						tag="tr"
						@change="changeGroup(records, $event, grouped.groupedBy)">
